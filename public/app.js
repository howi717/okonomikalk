...
function switchTab(group,id){if(group==="calc"&&id==="selvstendig"&&!isPremium()){setToolMode("private");switchPage("hjem");openModal();return}if(group==="calc")setToolMode(id==="selvstendig"?"business":"private");document.querySelectorAll(`[data-tab-group="${group}"]`).forEach(b=>b.classList.toggle("active",b.dataset.tab===id));document.querySelectorAll(`[data-tab-panel="${group}"]`).forEach(p=>p.classList.toggle("active-tab",p.id===`tab-${id}`))}
function openPrivateTools(){setToolMode("private");switchPage("kalkulatorer");switchTab("calc","lonnskatt")}
function openBusinessTools(){if(!isPremium()){setToolMode("private");switchPage("hjem");openModal();return}setToolMode("business");switchPage("kalkulatorer");switchTab("calc","selvstendig")}
...
function showLicenseNotice(licenseCode){
  ...
  box.innerHTML=`<div class="license-notice-card"><button class="modal-close" id="closeLicenseNotice" type="button">×</button><p class="eyebrow">Takk for kjøpet</p><h2>Næringsverktøy er åpnet</h2><p>Ta godt vare på lisenskoden din. Den kan brukes for å åpne næringsverktøy på mobil, nettbrett eller en annen nettleser.</p><input class="license-code-output" readonly value="${licenseCode}"><div class="form-actions"><button class="btn primary" id="copyLicenseCode" type="button">Kopier lisenskode</button><button class="btn ghost" id="continueAfterLicense" type="button">Fortsett</button></div><p class="fineprint">Når du trykker “Kopier lisenskode”, blir koden kopiert til utklippstavlen. Den ligger der bare til du kopierer noe annet, så det er lurt å skrive den ned eller lagre den et trygt sted. Koden lagres også lokalt i denne nettleseren.</p></div>`;
  ...
  document.getElementById("copyLicenseCode").addEventListener("click",async()=>{try{await navigator.clipboard.writeText(licenseCode);alert("Lisenskoden er kopiert til utklippstavlen. Husk å skrive den ned eller lagre den et trygt sted.")}catch{alert("Kopier lisenskoden manuelt.")}});
}
...
function invoiceDownloadHtml(inv){return `<!doctype html><html lang="no"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(inv.number)}</title><style>@page{size:A4;margin:12mm}*{box-sizing:border-box}html,body{margin:0;padding:0;background:#fff;font-family:Inter,Arial,sans-serif;color:#10233d;font-size:12px;line-height:1.35}.print-toolbar{position:sticky;top:0;z-index:5;display:flex;gap:10px;justify-content:center;padding:12px;background:#f5f7fb;border-bottom:1px solid #d7e0ea}.print-toolbar button{border:0;border-radius:999px;background:#0f766e;color:#fff;font-weight:900;padding:12px 18px;font-size:14px}.print-help{color:#66768a;font-size:13px;align-self:center}.wrap{width:100%;max-width:186mm;margin:0 auto;padding:10mm 0}.invoice-document{width:100%;border:0;border-radius:0;background:#fff;padding:0}h3{margin:0 0 10mm;font-size:30px;line-height:1;letter-spacing:-.04em}.invoice-party-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10mm;margin-bottom:8mm}.invoice-party-card{display:grid;align-content:start;gap:3px;min-height:28mm;padding:6mm;border:1px solid #d7e0ea;border-radius:8px;background:#f7f9fc}.invoice-card-label{color:#66768a;font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:.08em}.invoice-party-card strong{font-size:13px}.invoice-info-bar{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:5mm;margin-bottom:5mm}.invoice-info-bar>div,.invoice-account-bar{padding:4mm;border:1px solid #d7e0ea;border-radius:8px;background:#fff}.invoice-info-bar span,.invoice-account-bar span{display:block;margin-bottom:2px;color:#66768a;font-size:10px;font-weight:800}.invoice-account-bar{display:flex;justify-content:space-between;align-items:center;gap:8mm;margin-bottom:8mm}.invoice-table-wrap{border:1px solid #d7e0ea;border-radius:8px;overflow:hidden;margin:0}.invoice-table{width:100%;min-width:0;border-collapse:collapse;table-layout:fixed;background:#fff}.invoice-table th,.invoice-table td{padding:3.2mm 2.4mm;border-bottom:1px solid #d7e0ea;text-align:left;vertical-align:top;font-size:11px}.invoice-table th{background:#f8fbff;color:#66768a;font-size:10px}.invoice-table th:first-child,.invoice-table td:first-child{width:38%}.invoice-table th:nth-child(2),.invoice-table td:nth-child(2){width:10%}.invoice-table th:nth-child(3),.invoice-table td:nth-child(3){width:18%}.invoice-table th:nth-child(4),.invoice-table td:nth-child(4){width:10%}.invoice-table th:nth-child(5),.invoice-table td:nth-child(5){width:24%}.invoice-table th:nth-child(n+2),.invoice-table td:nth-child(n+2){text-align:right;white-space:nowrap}.invoice-totals{display:grid;gap:3mm;margin-top:6mm;margin-left:auto;width:75mm}.invoice-totals>div{display:flex;justify-content:space-between;gap:8mm;padding:3.2mm 4mm;border:1px solid #d7e0ea;border-radius:8px;background:#f7f9fc}.invoice-totals>div:last-child{background:#eef7f5;border-color:rgba(15,118,110,.28);font-size:13px}@media print{html,body{background:#fff}.print-toolbar{display:none!important}.wrap{max-width:none;padding:0}.invoice-document{break-inside:avoid;page-break-inside:avoid}}@media(max-width:760px){.print-toolbar{display:grid}.print-help{text-align:center}.wrap{padding:8mm 4mm}.invoice-party-grid,.invoice-info-bar{grid-template-columns:1fr}.invoice-account-bar{display:grid}.invoice-table{font-size:10px}}</style></head><body><div class="print-toolbar"><button onclick="window.print()">Skriv ut / lagre som PDF</button><span class="print-help">Denne siden inneholder kun fakturaen.</span></div><div class="wrap">${invoiceHtml(inv)}</div><script>setTimeout(function(){try{window.focus();window.print();}catch(e){}},350);<\/script></body></html>`}
function printInvoicePdf(){
  if(!isPremium()){openModal();return}
  const inv=readInvoice();
  const html=invoiceHtml(inv);
  const id=`${Date.now()}-${Math.random().toString(36).slice(2)}`;
  try{
    localStorage.setItem(`okonomikalk_invoice_print_${id}`,JSON.stringify({html,number:inv.number,createdAt:new Date().toISOString()}));
  }catch(error){
    alert("Kunne ikke klargjøre faktura for utskrift. Prøv igjen.");
    return;
  }
  const url=`/faktura-utskrift/?id=${encodeURIComponent(id)}`;
