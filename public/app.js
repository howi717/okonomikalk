...
function printInvoicePdf(){if(!isPremium()){openModal();return}const inv=readInvoice();const html=invoiceDownloadHtml(inv);const printWindow=window.open("","_blank","noopener,noreferrer");if(!printWindow){alert("Nettleseren blokkerte utskriftsvinduet. Tillat popup for okonomikalk.no og prøv igjen.");return}printWindow.document.open();printWindow.document.write(html);printWindow.document.close()}
