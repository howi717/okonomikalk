...
const PRODUCT_KEY = "okonomikalk_naering_199";
...
function siteLock(req, res, next) {
  ...
  // Stripe webhook må få slippe gjennom hvis webhook brukes.
...
// Midlertidig lås for hele siden mens du tester.
app.use(siteLock);
...
app.post("/api/validate-license", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Could not validate license." });
  }
});
...
app.listen(PORT, () => {
  console.log(`ØkonomiKalk running on ${PUBLIC_URL}`);
